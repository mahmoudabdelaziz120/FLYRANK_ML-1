...
- **Excluded Columns (Leakage Isolation):**
  ...
  - *Mathematical Overlap Leakage:* We audited the 90-day aggregate columns and mathematically confirmed that $\text{impressions\_90d} - (\text{impressions\_prev\_30d} + \text{impressions\_last\_30d}) \ge 0$. Retaining 90-day metrics allowed decision trees to algebraically infer the target window via simple subtraction ($\text{ROC-AUC}$ falsely inflated to $\approx 0.80$). Consequently, all cumulative 90-day aggregates (`*_90d`), activity counters (`days_with_impressions`, `days_with_sessions`), and derived 90-day ratios (`ctr`, `avg_position`, `engagement_rate`, `scroll_rate`, `ai_traffic_pct`, `impression_tier`, `position_tier`) were stripped from the feature space[cite: 1].
